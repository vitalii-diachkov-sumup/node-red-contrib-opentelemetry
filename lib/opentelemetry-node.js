...
function logEvent (node, eventType, event) {
  ...
  try {
    ...
    console.log(`[OpenTelemetry] ${eventType}: ${msg}`)
  } catch (error) {
    console.error(`[OpenTelemetry] An error occurred during logging ${eventType}`, error)
  }
}
  ...
  try {
    for (const [msgId, msgSpan] of msgSpans) {
      if (msgSpan.updateTimestamp < now - _timeout) {
        ...
        if (_isLogging) {
          console.log(`[OpenTelemetry] Parent span "${msgSpan.parentSpan.name}" ${msgId} is outdated, ending`)
        }
    ...
    console.error('[OpenTelemetry] An error occurred during span cleaning', error)
  }
}
  ...
  _attributeMappings
    ...
    .forEach((mapping) => {
      try {
        ...
        console.warn(`[OpenTelemetry] An error occurred during span attribute parsing (key: ${mapping.key}, path: ${mapping.path}): ${error.message}`)
      }
    })
  return attributes
}
 ...
 * @param {any} nodeDefinition Current node definition
 * @param {any} node OTEL node (for using Node-RED utilities)
 * @param {boolean} isNotTraced Should the node be traced?
 * @returns {Span|undefined} Created span
 */
function createSpan (tracer, msg, nodeDefinition, node, isNotTraced) {
  try {
    // check and get message id (handle root message id for split parts)
    ...
    if (msgSpans.has(msgId)) {
      ...
      if (_isLogging) {
        console.log(`[OpenTelemetry] => Created parent span for ${nodeDefinition.type}`)
      }
    }
    ...
    if (_isLogging) {
      console.log(`[OpenTelemetry] Local span attributes (start) for ${nodeDefinition.id}, ${nodeDefinition.type}: ${localAttributes ? JSON.stringify(localAttributes) : 'none'}`)
    }
    ...
    if (_isLogging) {
      console.log(`[OpenTelemetry] => Created span for ${nodeDefinition.type}`)
    }
    ...
    console.error('[OpenTelemetry] An error occurred during span creation', error)
  }
}
...
function endSpan (msg, error, nodeDefinition) {
  try {
    ...
    // api-call node is always setting msg.statusCode to 200 for all responses
    // so the only way to check if the error has occurred is to inspect msg.payload
    // error property
    //
    // `api_call` node is also supposed to write additional msg._otel object
    if (nodeDefinition.type === 'api-call') {
      if (_isLogging) {
        console.log(`[OpenTelemetry] Processing api-call node. msg._otel exists: ${!!msg._otel}`)
        if (msg._otel) {
          try {
            const safeOtelData = {
              isError: msg._otel.isError,
              originalErrorMessage: msg._otel.originalErrorMessage,
              response: msg._otel.response ? {
                status: msg._otel.response.status,
                statusText: msg._otel.response.statusText,
                config: {
                  url: msg._otel.response.config?.url,
                  method: msg._otel.response.config?.method
                }
              } : null
            }
            console.log('[OpenTelemetry] api-call _otel data:', JSON.stringify(safeOtelData))
          } catch (jsonError) {
            console.log('[OpenTelemetry] api-call _otel data: (unable to stringify, circular refs detected)')
          }
        }
      }
      if (msg._otel) {
        if (msg._otel.isError) {
          span.setStatus({ code: SpanStatusCode.ERROR })
          span.setAttribute('axios.error_message', msg._otel.originalErrorMessage)
          span.setAttribute('error.message', msg._otel.originalErrorMessage)
          span.setAttribute('error.type', 'network_error')
        } else {
          span.setStatus({ code: SpanStatusCode.OK })
        }

        const response = msg._otel.response
        if (response) {
          span.setAttribute(ATTR_HTTP_RESPONSE_STATUS_CODE, response.status)

          // Check if response status indicates an error (4xx or 5xx)
          const isBadResponse = response.status >= 400
          if (isBadResponse) {
            span.setStatus({ code: SpanStatusCode.ERROR })
            
            // Add error indicator for bad HTTP responses (no sensitive data)
            const httpErrorMessage = `HTTP ${response.status} ${response.statusText || 'Error'}`
            span.setAttribute('error.message', httpErrorMessage)
            span.setAttribute('http.error_message', httpErrorMessage)
            span.setAttribute('error.type', 'http_error')
            
            // Log detailed error response to console with msg._msgid for correlation
            if (response.data !== undefined) {
              try {
                const responseContent = typeof response.data === 'string' 
                  ? response.data 
                  : JSON.stringify(response.data)
                console.log(`[OpenTelemetry] api-call error - msgId: ${msg._msgid}, status: ${response.status}, response: ${responseContent}`)
              } catch (stringifyError) {
                console.log(`[OpenTelemetry] api-call error - msgId: ${msg._msgid}, status: ${response.status}, response: [Unable to serialize response data]`)
              }
            } else {
              console.log(`[OpenTelemetry] api-call error - msgId: ${msg._msgid}, status: ${response.status}, response: [No response data available]`)
            }
          }
          // add URL info in attributes
          try {
        ...
        } else if (msg._otel.isError) {
          // Handle cases where there's no response object (network errors, timeouts, etc.)
          span.setAttribute('network.error_message', msg._otel.originalErrorMessage)
          
          console.log(`[OpenTelemetry] api-call network error - msgId: ${msg._msgid}, error: ${msg._otel.originalErrorMessage}`)
        }
      }
    }
    ...
    if (localAttributes !== undefined) {
      ...
      if (_isLogging) {
        console.log(`[OpenTelemetry] Local span attributes (end) for ${nodeDefinition.id}, ${nodeDefinition.type}: ${localAttributes ? JSON.stringify(localAttributes) : 'none'}`)
      }
    }
    ...
    if (nodeDefinition.type === 'http response') {
      ...
      for (const [msgSpanId, spanIn] of parent.spans) {
        if (spanIn.attributes && spanIn.attributes['node_red.node.type'] === 'http in') {
          if (_isLogging) {
            console.log(`[OpenTelemetry] ==> Ended related span for ${msgSpanId} http in`)
          }
          spanIn.end()
    ...
    if (_isLogging) {
      console.log(`[OpenTelemetry] ==> Ended span for ${nodeDefinition.id} ${nodeDefinition.type}`)
    }
    parent.spans.delete(msgSpanId)
    ...
    for (const [, span] of parent.spans) {
      if (!ORPHAN_NODE_TYPES.includes(span.attributes && span.attributes['node_red.node.type']) || span._creationTimestamp > currentSpanCreationTimestamp) {
    ...
    if (isOrphan) {
      for (const [msgSpanId] of parent.spans) {
        if (_isLogging) {
          console.log(`[OpenTelemetry] Orphan span to delete: ${msgSpanId}`)
        }
        parent.spans.delete(msgSpanId)
      }
      // all children are completed, end and remove parent span
      if (_isLogging) {
        console.log(`[OpenTelemetry] Parent span "${parent.parentSpan.name}" no longer has child span, ending`)
      }
      parent.parentSpan.end()
      msgSpans.delete(msgId)
    }
  } catch (error) {
    console.error('[OpenTelemetry] Error in endSpan:', error)
  }
}
...
module.exports = function (RED) {
  ...
  function OpenTelemetryNode (config) {
    ...
    // get config
    const { url, protocol, serviceName, rootPrefix, ignoredTypes, ignoredFlows, propagateHeadersTypes, isLogging, timeout, attributeMappings } = config
    const ignoredTypesList = ignoredTypes.split(',').map(key => key.trim())
    const ignoredFlowsList = ignoredFlows.split(',').map(key => key.trim())
    ...
    /**
      * Check if given node should be ignored by OTEL tracing
      * @param {any} nodeDefinition - node definition from the flow
      * @returns {boolean} true if node should be ignored
      */
    function isNodeIgnored (nodeDefinition) {
      const shouldIgnore = ignoredTypesList.includes(nodeDefinition.type)
      if (shouldIgnore && _isLogging) {
        RED.log.debug(`[OpenTelemetry] Ignoring node (id: ${nodeDefinition.id}, type: ${nodeDefinition.type}, z: ${nodeDefinition.z})`)
      }
      return shouldIgnore
    }

    /**
      * Check if given flow should be ignored
      * @param {string} flowID - flow that has to be checked
      * @returns {boolean} - true if should be ignored
      */
    function isFlowIgnored (flowID) {
      const shouldIgnore = ignoredFlowsList.includes(flowID)
      if (shouldIgnore && _isLogging) {
        RED.log.debug(`[OpenTelemetry] Ignoring nodes from flow "${flowID}"`)
      }
      return shouldIgnore
    }
    ...
    // create tracer
    const traceExporterProps = {
      ...(url && { url }),
      ...(process.env.OTEL_EXPORTER_OTLP_TIMEOUT && { timeoutMillis: parseInt(process.env.OTEL_EXPORTER_OTLP_TIMEOUT) }),
    }

    let traceExporter
    if (protocol === 'proto') {
      const { OTLPTraceExporter } = require('@opentelemetry/exporter-trace-otlp-proto')
      traceExporter = new OTLPTraceExporter(traceExporterProps)
    } else {
    ...
    const spanProcessor = new BatchSpanProcessor(traceExporter, {
      ...
      maxExportBatchSize: parseInt(process.env.OTEL_BSP_MAX_EXPORT_BATCH_SIZE) || 10,
    ...
    traceExporter.export = function (spans, resultCallback) {
      if (_isLogging) {
        RED.log.info(`[OpenTelemetry] Attempting to export ${spans.length} spans to ${traceExporterURL}`)
      }
      const wrappedCallback = (result) => {
        if (result.code === 0) {
          if (_isLogging) {
            RED.log.info(`[OpenTelemetry] Successfully exported ${spans.length} spans`)
          }
        } else {
          RED.log.error(`[OpenTelemetry] Failed to export spans: ${result.error || 'Unknown error'}`)
        }
    ...
    if (_isLogging) {
      RED.log.info(`[OpenTelemetry] Initializing with trace exporter props: ${JSON.stringify(traceExporterProps)}`)
      RED.log.info(`[OpenTelemetry] Using protocol: ${protocol}`)
      RED.log.info(`[OpenTelemetry] Target URL: ${traceExporterURL}`)
      RED.log.info('[OpenTelemetry] Environment variables:')
      RED.log.info(`  OTEL_EXPORTER_OTLP_ENDPOINT: ${process.env.OTEL_EXPORTER_OTLP_ENDPOINT || 'not set'}`)
      RED.log.info(`  OTEL_EXPORTER_OTLP_TIMEOUT: ${process.env.OTEL_EXPORTER_OTLP_TIMEOUT || 'not set'}`)
      RED.log.info(`  OTEL_RESOURCE_ATTRIBUTES: ${process.env.OTEL_RESOURCE_ATTRIBUTES || 'not set'}`)
    }
    ...
    if (_isLogging) {
      RED.log.info(`[OpenTelemetry] Initializing tracer provider with resource: ${JSON.stringify(mergedResource.attributes)}`)
    }
    ...
    if (_isLogging) {
      RED.log.info('[OpenTelemetry] Tracer provider registered successfully')
      RED.log.info(`[OpenTelemetry] Using tracer: ${name} v${version}`)
    }
    ...
    RED.hooks.add('onSend.otel', (events) => {
      ...
      const nodeDefinition = events[0].source.node
      if (!isFlowIgnored(nodeDefinition.z)) {
        createSpan(tracer, events[0].msg, nodeDefinition, node, isNodeIgnored(nodeDefinition))
      }
    })
    ...
    RED.hooks.add('postDeliver.otel', (sendEvent) => {
      ...
      const nodeDefinition = sendEvent.destination.node
      if (!isFlowIgnored(nodeDefinition.z)) {
        const span = createSpan(tracer, sendEvent.msg, nodeDefinition, node, isNodeIgnored(nodeDefinition))
        ...
        if (sendEvent.source.node.type === 'switch' || sendEvent.source.node.type.startsWith('subflow')) {
          ...
          if (parent && parent.spans.has(spanId)) {
            if (_isLogging) {
              console.log(`[OpenTelemetry] Switch or subflow span ${spanId} will be ended`)
            }
    ...
    RED.hooks.add('onComplete.otel', (completeEvent) => {
      ...
      const nodeDefinition = completeEvent.node.node
      if (!isFlowIgnored(nodeDefinition.z)) {
        endSpan(completeEvent.msg, completeEvent.error, nodeDefinition)
      }
    })
