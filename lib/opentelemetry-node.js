...
function endSpan (msg, error, nodeDefinition) {
  try {
    ...
    if (nodeDefinition.type === 'api-call' && msg._otel) {
      console.log('msg._otel:', msg._otel)
      if (msg._otel.isError) {
        span.setStatus({ code: SpanStatusCode.ERROR })
        span.setAttribute('axios.error_message', msg._otel.errorMessage)
      } else {
        span.setStatus({ code: SpanStatusCode.OK })
      }
  ...
  function OpenTelemetryNode (config) {
    ...
    function isNodeIgnored (nodeDefinition) {
      ...
      if (shouldIgnore && _isLogging) {
        console.log(`Ignoring node (id: ${nodeDefinition.id}, type: ${nodeDefinition.type}, z: ${nodeDefinition.z})`)
      }
      return shouldIgnore
    }
    ...
    function isFlowIgnored (flowID) {
      ...
      if (shouldIgnore && _isLogging) {
        console.log(`Ignoring nodes from flow "${flowID}"`)
      }
      return shouldIgnore
    }
    ...
    if (_isLogging) {
      console.log('Initializing with trace exporter props:', traceExporterProps)
    }
